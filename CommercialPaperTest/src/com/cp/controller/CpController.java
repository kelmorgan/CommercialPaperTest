...
import com.cp.api.execute.Api;
import com.cp.api.generateXml.RequestXml;
import com.cp.utils.Shared;
import com.cp.utils.Constants;
import com.cp.utils.LogGenerator;
import com.cp.utils.XmlParser;
...
public class CpController implements Constants {
    private static final Logger logger = LogGenerator.getLoggerInstance(CpController.class);
    ...
    public String getPostTxn(String acct1, String sol1, String amount, String transParticulars, String partTranRemarks, String acct2, String sol2){
        ...
        try {
            if (Integer.parseInt(amount) > 0) {
                outputXml = Api.executeCall(postServiceName, RequestXml.postTransactionXml(transType, transSubTypeC, acct1, sol1, debitFlag, amount, currencyNgn, transParticulars, partTranRemarks, Shared.getCurrentDate(), acct2, sol2, creditFlag, Shared.getLoginUser(ifr)));
                logger.info("outputXml-- "+outputXml);
                if (!Shared.isEmpty(outputXml)) {
                    xmlParser.setInputXML(outputXml);
                    String status = xmlParser.getValueOf(apiStatus);
    ...
    public String fetchAcctDetails(){
        try {
            ...
            String accountNumber = Shared.getCpAcctNo(ifr).trim();
            logger.info("account Number-- "+ accountNumber );
            Shared.clearFields(ifr, new String[]{cpCustomerSolLocal, cpCustomerEmailLocal, cpCustomerNameLocal});
                ...
                if (!Shared.isEmpty(outputXml)) {
                    ...
                    if (isSuccess(status)) {
                        ...
                        if (isSchemeCodeInvalid(schemeCode)) {
                            Shared.clearFields(ifr, new String[]{cpCustomerAcctNoLocal, cpCustomerNameLocal, cpCustomerEmailLocal, cpLienStatusLocal});
                        ...
                        if (Shared.isEmpty(email)) {
                            Shared.setFields(ifr, new String[]{cpCustomerNameLocal, cpCustomerSolLocal}, new String[]{name, sol});
                            Shared.enableFields(ifr, new String[]{cpCustomerEmailLocal});
                            ...
                            Shared.disableFields(ifr, new String[]{cpCustomerEmailLocal});
                        }
                    } else if (isFailed(status)) {
                        String errCode = xmlParser.getValueOf("ErrorCode");
                        String errDesc = xmlParser.getValueOf("ErrorDesc");
    ...
    public String fetchLien(){
        try {
            ...
            outputXml = Api.executeCall(fetchLienServiceName, RequestXml.fetchLienRequestXml(Shared.getCpAcctNo(ifr).trim()));
            logger.info("outputXml-- " + outputXml);
            if (!Shared.isEmpty(outputXml)) {
                xmlParser.setInputXML(outputXml);
                String status = xmlParser.getValueOf(apiStatus);
                if (isSuccess(status)) {
                    ...
                    if (!Shared.isEmpty(lienId))
                        Shared.setFields(ifr, cpLienStatusLocal, yes);
                } else if (isFailed(status)) {
    ...
    public String tokenValidation(String otp){
        ...
        if (!otp.isEmpty()){
            outputXml = Api.executeCall(tokenValidationServiceName,RequestXml.tokenValidationXml(Shared.getLoginUser(ifr),otp));
            logger.info("outputXml-- "+outputXml);
